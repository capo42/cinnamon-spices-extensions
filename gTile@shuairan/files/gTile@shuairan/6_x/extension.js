...
;// ../base/ui/AutoTileMainAndList.ts
...
;// ../base/ui/GridElementDelegate.ts
...
;// ../base/ui/ToggleSettingsButton.ts
...
class App {
        ...
        this.ResetFocusedWindow = () => {
            ...
            if (this.focusMetaWindowConnections.length > 0) {
                for (const id of this.focusMetaWindowConnections) {
                    (_a = this.focusMetaWindow) === null || _a === void 0 ? void 0 : _a.disconnect(id);
                }
            }
    ...
    destroy() {
        ...
        if (this.area.get_parent() === app_Main.uiGroup) {
            app_Main.uiGroup.remove_actor(this.area);
        }
        this.config.destroy();
        this.DestroyGrid();
        this.ResetFocusedWindow();
...
const createExtension = (platform) => {
    let metadata;
    let app;
    const init = (meta) => {
        metadata = meta;
